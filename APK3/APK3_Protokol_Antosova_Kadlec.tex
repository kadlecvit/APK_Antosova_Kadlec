...
\textbf{Ostré hřbety a údolnice} -- Delaunay triangulace nezná pojem charakteristické čáry terénu (hřbet, údolnice, hrana). Pokud nejsou tyto linie vyznačeny vstupními body dostatečně hustě, algoritmus je neidentifikuje a vyhladí. Trojúhelníky pak kříží hřebenovou linii a spojují body ze severního i jižního svahu v rámci jednoho trojúhelníku. Taková plocha má nerealistický sklon a hlavně nesprávnou expozici -- namísto odlišených severních a jižních svahů vznikají trojúhelníky orientované libovolně napříč hřebenem. Pro ilustraci byl vygenerován syntetický soubor bodů s ostrým hřbetem uprostřed území. Na obrázku \ref{fig:hrbet} je v analýze expozice patrné, jak různobarevné trojúhelníky chaoticky střídají orientaci přes oblast hřebene, místo aby se plynule přecházelo ze severní expozice (zelená) na jižní (růžová).

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{problematicka_mista_hrbety.png}
\caption{\textit{Slabina DT: ostrý hřbet}}
\label{fig:hrbet}
\end{figure}

\textbf{Nerovnoměrně rozmístěné body} -- V oblastech s řídkým rozmístěním bodů mohou vznikat velké trojúhelníky, které hrubě aproximují terén. Sklon a expozice těchto rozsáhlých ploch pak nemusí odpovídat skutečnosti. Na obrázku \ref{fig:nerovnomerne} je tato slabina patrná v severní části území, kde jsou body rozmístěny řídce a vznikají výrazně větší trojúhelníky táhnoucí se podél severního okraje. Jejich sklon a expozice jsou nerealistické, protože každý takový trojúhelník pokrývá příliš velkou plochu s nedostatečnou informací o skutečném průběhu terénu.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{problematicka_mista.png}
\caption{\textit{Slabina DT: Degenerované trojúhelníky na okraji}}
\label{fig:nerovnomerne}
\end{figure}

\textbf{Degenerované trojúhelníky na okraji konvexní obálky} -- Na hranici vstupní množiny bodů, kde jsou body rozmístěny řídce, vznikají velmi dlouhé a úzké trojúhelníky táhnoucí se podél okraje. Tento jev je také patrný na obrázku \ref{fig:nerovnomerne}.
