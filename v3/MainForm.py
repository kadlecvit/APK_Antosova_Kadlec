...
class Ui_MainForm(object):
    ...
    def openClick(self):
        #User defined slot, open shapefile
    ...
    def analyzePointAndPositionRCClick(self):
        q = self.Canvas.getQ()
        polygons = self.Canvas.getPolWithHoles()
        a = Algorithms()

        result_text = "Point is outside all polygons"
        for i, (pol, holes) in enumerate(polygons):
            res = a.getPointPolygonPositionRC(q, pol)
            label = f"Polygon {i + 1}"

            if res == "vertex":
                result_text = f"{label}: Point is on a vertex of the polygon"
                break
            elif res == "edge":
                result_text = f"{label}: Point is on an edge of the polygon"
                break
            elif res == 1:
                # Check holes
                hole_text = None
                for hole in holes:
                    hr = a.getPointPolygonPositionRC(q, hole)
                    if hr == "vertex":
                        hole_text = f"{label}: Point is on a vertex of a hole"
                        break
                    elif hr == "edge":
                        hole_text = f"{label}: Point is on an edge of a hole"
                        break
                    elif hr == 1:
                        hole_text = ""  # inside hole = outside polygon
                        break
                if hole_text is None:
                    result_text = f"{label}: Point is inside the polygon"
                    break
                elif hole_text:
                    result_text = hole_text
                    break
                # else: inside hole, treat as outside -> continue to next polygon
    ...
    def analyzePointAndPositionWNClick(self):
        ...
        for i, (pol, holes) in enumerate(polygons):
            ...
            if res == "vertex":
                ...
                hole_text = None
                for hole in holes:
                    hr = a.getPointPolygonPositionWN(q, hole)
                    if hr == "vertex":
                        hole_text = f"{label}: Point is on a vertex of a hole\nΩ = vertex"
                        break
                    elif hr == "edge":
